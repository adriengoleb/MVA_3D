...
#
def transform(p):
    mean = np.mean(p, axis=0) #Center the cloud on its centroid
    centered_p = p - mean
    scaled_p = centered_p / 2
    recenter_p = scaled_p + mean
    recenter_p[:,1] = recenter_p[:,1] - 0.1 #Apply a -10cm translation along y-axis.
    return recenter_p
# ------------------------------------------------------------------------------------------
#
#           Main
...
if __name__ == '__main__':
    ...
    #we call the transform function
    transformed_points = transform(points)
    print(points == transformed_points) #and we print the outpout to verify
    # Save point cloud
    # *********************
    #
